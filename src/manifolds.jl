...
export TorusEmbedding,
    SphereEmbedding, visualize_manifold, MobiusEmbedding, CylinderEmbedding
...
function (e::AbstractEmbedding)(
    x::AbstractVector,
    y::AbstractVector,
)::Tuple{Matrix,Matrix,Matrix}
    pts = [e.φ([x, y]) for x in x, y in y]
    X = [p[1] for p in pts]
    Y = [p[2] for p in pts]
    Z = [p[3] for p in pts]
...
@with_kw struct TorusEmbedding <: AbstractEmbedding
    φ::Function =
        (p) -> begin
            R, r = 1.0, 0.5
            x, y = p
            return [(R + r * cos(x)) * cos(y), (R + r * cos(x)) * sin(y), r * sin(x)]
        end
end
...
theme = Theme(Axis3d = (backgroundcolor = :black, xgridcolor = :white, ygridcolor = :white))
