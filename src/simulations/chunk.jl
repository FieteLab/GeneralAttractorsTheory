...
function RandomChunk(
    ...
    η = moving_average(rand(Float64, nframes), 25)
    θ =
        sin.((1:nframes) ./ (100 / simulation.dt)) .-
        sin.((1:nframes) ./ (75 / simulation.dt)) .+
        sin.((1:nframes) ./ (125 / simulation.dt)) .+
        sin.((1:nframes) ./ (512 / simulation.dt)) .+
        sin.((1:nframes) ./ (50 / simulation.dt)) .+ η


    μ = μ₀ .+ (moving_average(rand(Float64, nframes), smoothing_window) .- 0.5) .* σ
...
function Plots.plot(chunk::RandomChunk; kwargs...)
    ...
    plt = if d == 1
        ...
        θ = acos.(clamp.(map(x -> x[1], chunk.v), -1, 1))
        p3 = plot(θ, label = "θ")
        plot(p1, p2, p3)
    end
    display(plt)
