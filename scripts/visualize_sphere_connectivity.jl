...
for i in 1:2:5
    ...
    p = Plots.scatter3d(eachrow(spherecan.X)..., 
        marker_z=conn, markersize=4,  msa=0, msw=0, alpha=1, 
        label=nothing, camera=(90, 15), color=:bwr, xlim=[0, 1.5],
        )

    scatter3d!([spherecan.X[1, idx]], [spherecan.X[2, idx]], [spherecan.X[3, idx]] ,ms=8, color=:black)
