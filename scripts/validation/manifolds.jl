...
@with_kw struct Sphere <: AbstractPointManifold
    d::Int = 2
    size::Vector = [2π, 2π]
    ϕ::Function = (p0, p1) -> [sin(p0) * cos(p1), sin(p0) * sin(p1), cos(p0)]
end

@with_kw struct Cylinder <: AbstractPointManifold
    d::Int = 2
    size::Vector = [2π, 1]
    ϕ::Function = (p0, p1) -> [sin(p0), cos(p0), 2p1 - 1]
end
...
@with_kw struct Ring <: AbstractPointManifold
    d::Int = 1
    size::Vector = [2π]
    ϕ::Function = (p) -> [sin(p), cos(p), cos(p)]
end
...
struct Sphere_ℝᵈ <: HigherDimensionalManifold
    ...
    function Sphere_ℝᵈ(k::Int; N = 1000, η = 0.0)
        ...
        u = mapslices(x -> x ./ norm(x), u; dims = 1)
...
function generate_manifold_pointcloud(m::HigherDimensionalManifold; N = 1000, η = 0.1)
...
function gram_schmidt(P::Matrix)::Matrix
    basis = []
    for v in eachrow(P)
        w = isempty(basis) ? v : v .- sum(map(b -> v ⋅ b, basis))
        push!(basis, w / norm(w))
    end
