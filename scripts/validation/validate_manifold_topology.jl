...
function validate_manifold(manifold::AbstractPointManifold)
    ...
    for (n, η) in enumerate(noise)
        ...
        D = zeros(20, 25)
        for (i, nsize) in enumerate(local_pca_nsize),
            (j, npoints) in enumerate(local_pca_npoints)
    ...
    @info "plotting"
    p = plot(plots..., size = (1400, 1400), layout = (5, 3))
    savefig(savepath("manifold_validation", string(manifold), "png"))
    return plots
end
