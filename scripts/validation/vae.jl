...
encoder = Chain(Dense(K => inner, tanh), Dense(inner => D, tanh))

decoder = Chain(Dense(D => inner, tanh), Dense(inner => K, tanh))
...
L = []
@track for i = 1:2500
    Flux.Optimise.train!(ℓ, θ, X, opt)

    l = mean(ℓ.(X))
    push!(L, l)
    l < 0.001 && break
end
plot(L, ylim = [0, maximum(L) + 0.2]) |> display


# -------------------------- plot reconstructed data ------------------------- #
p = plot()
scatter!(eachrow(M)..., ms = 5, color = :black, alpha = 0.5)
Y = hcat(vae.(X)...)
scatter!(
    eachrow(Y)...,
    color = :red,
...
for v in range(-1, 1, length = 15)
    ...
    scatter!(eachrow(ŷ₁)..., alpha = 0.5, label = nothing, color = :red)
    scatter!(eachrow(ŷ₂)..., alpha = 0.5, label = nothing, color = :black)
end
plot(p1, p2, plot(p2, camera = (0, 90)), plot(p2, camera = (90, 0)), size = (800, 800)) |>
display
